...
sig 
  ...
  val isa_dtyp_to_term : Old_Datatype_Aux.info -> term
...
(* ---------------------------------------------*) 
fun isa_elem_to_term descr e =
    case e of
      Old_Datatype_Aux.DtTFree (nm,sort) => Var((nm,0),@{typ "ACFilterDtyp.type"})
    | Old_Datatype_Aux.DtType (nm, dtyps) =>
      let val typnm =
              (Old_Datatype_Aux.DtType (nm, dtyps))
              |> Old_Datatype_Aux.typ_of_dtyp descr
              |> Old_Datatype_Aux.name_of_typ
      in
        @{term "Typ"} $ Var((typnm,0),@{typ "string"})
      end
    | Old_Datatype_Aux.DtRec i => @{term "Recursive"};

(* Translate an Isabelle datatype to our term-representation *)
fun isa_elems_to_term (descr) [] = @{term "Unit"}
  | isa_elems_to_term (descr) [e] = isa_elem_to_term (descr) e
  | isa_elems_to_term (descr) (e::es) = 
    let val ts = isa_elems_to_term (descr) es 
    in @{term "Times"}$(isa_elem_to_term (descr) e)$ts end;

fun isa_constr_dtyps_to_term (descr) ([]) = raise ERROR "empty type"
    | isa_constr_dtyps_to_term (descr) ([(_,elems)]) = 
      isa_elems_to_term (descr) elems
    | isa_constr_dtyps_to_term (descr) ((_,elems)::cs) =
      let 
        val ts = isa_constr_dtyps_to_term (descr) cs
        val els = isa_elems_to_term (descr) elems  
      in @{term "Plus"}$els$ts end;  

(* Make one of our terms from an Isabelle datatype *)
fun isa_dtyp_to_term (dinfo : Old_Datatype_Aux.info) =
    let 
      val descr = (#descr dinfo)
      (*A list of Isabelle-dtyps, as (string, dtyp list) pairs*)
      val dtyp_constrs = 
          maps (fn (_,(_,_, constructor_name)) => constructor_name) descr
    in
      isa_constr_dtyps_to_term descr dtyp_constrs
    end;
...
val isabelle_datatype_names = ["List.list", "Nat.nat","ACFilterDtyp.bin_tree", "ACFilterDtyp.bin_tree2", "ACFilterDtyp.bin_tree3"]; 
