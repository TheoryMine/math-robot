         ...
         let val result =
                 (if (FastArgNeglect.syntactically_arg_neglecting_fundef eqs)
                  ...
                  handle ERROR s => (SynthBug("ERROR exception: " ^ s))
                       | _ => (SynthBug("Unkown error total_count: " ^
                                 (Int.toString (!total_count))))
         in (total_count := !total_count +1;
             case result of
               Synthesised(fnm,thy2) =>
               (goodn := !goodn + 1; appfun thy2 fnm; thy2)
             | SynthBadFun(err_msg,thy2) =>
               (ignoredn := !ignoredn + 1; writeln err_msg; thy2)
             | SynthBug(err_msg) =>
               (ignoredn := !ignoredn + 1; writeln err_msg; thy)
             | SynthTrivial =>
               (ignoredn := !ignoredn + 1; thy))
         end)
