...
fun dest_eq t =
  let val z = Zipper.mktop t
  in
  (Zipper.trm (Subst.move_to_lhs_of_eq z),
...
fun normalise_arguments eq =
    let
      ...
      val (_,args) = eq' |> (HOLogic.dest_eq o HOLogic.dest_Trueprop)
                         |> fst |> Term.strip_comb
      fun norm_arg _ eq [] = eq
        | norm_arg i eq (arg :: args) = (case arg
              of Var (_,typ) => norm_arg (i+1)
                  (Term.subst_free [(arg,Var (("x",i),typ))] eq) args
...
fun is_step_case eq =
    ...
    in Term.exists_subterm (fn t =>
      (is_equal o Term_Ord.fast_term_ord) (elhead, t)) er end

(* Separates base cases and step cases *)
fun base_and_step_cases_of_eqns eqns =
    fold (fn a => fn (l,r) =>
      if is_step_case a then (l,a :: r) else (a :: l,r)) eqns ([],[])
...
fun pattern_arg_neglecting_fundef eqns =
    let
      ...
      fun check_neglected var eqn =
          ...
          in Term.exists_subterm (fn t =>
            (is_equal o Term_Ord.fast_term_ord) (var, t)) el andalso
             not (Term.exists_subterm (fn t =>
               (is_equal o Term_Ord.fast_term_ord) (var, t)) er) end
...
ML {*
(* Need to keep track of index of ignored arg *)
let val bad_defs = [@{prop "f 0 b = b"},
                    @{prop "f (Suc x) b = f x b"}];
val false = FastArgNeglect.syntactically_arg_neglecting_fundef bad_defs

(* TODO: xTrue should be true, but is currently false. *)
val xTrue = FastArgNeglect.pattern_arg_neglecting_fundef bad_defs
in (writeln "test passed") end;
*}
...
let val good_defs = [@{prop "f 0 b c = b"},
                     @{prop "f (Suc x) b c = f x b c"}];
val false = FastArgNeglect.syntactically_arg_neglecting_fundef good_defs

(* TODO: xTrue should be true, but is currently false. *)
val xTrue = FastArgNeglect.pattern_arg_neglecting_fundef good_defs
in (writeln "test passed") end;
*}
